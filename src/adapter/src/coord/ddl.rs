    ...
    pub async fn drop_compute_sinks(
        ...
        for sink_id in sink_ids {
            let sink = match self.remove_active_compute_sink(sink_id).await {
                None => {
                    // This can happen due to a race condition: an internal subscribe
                    // (e.g., read-then-write OCC) may be cleaned up via its own message
                    // while session disconnect cleanup is in progress. This is benign.
