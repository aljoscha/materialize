...
impl Coordinator {
    ...
    pub(crate) async fn catalog_transact_inner(
        ...
        // Broadcast the updated catalog to worker threads via the watch channel,
        // so they can read it directly without a coordinator round-trip.
        self.catalog_watch_tx
            .send_replace(Arc::clone(&self.catalog));
