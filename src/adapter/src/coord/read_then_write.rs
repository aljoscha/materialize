...
//! Handler functions for read-then-write (DELETE/UPDATE) operations.
//!
//! These support the frontend-sequenced OCC implementation in `frontend_read_then_write.rs`.
...
impl Coordinator {
    /// Handle a Command to create a read-then-write subscribe.
    ///
    /// This is called from the frontend OCC implementation after it has acquired the semaphore
    /// permit. We create the subscribe here (on the coordinator) and return the channel to the
    /// caller.
    ///
    /// The `read_holds` parameter contains the read holds for this specific operation.
    /// They are passed directly through the stages (not via the connection-keyed txn_read_holds
    /// map) to avoid issues where multiple operations on the same connection could interfere
    /// with each other's holds.
    #[allow(clippy::too_many_arguments)]
    pub(crate) async fn handle_create_read_then_write_subscribe(
        &mut self,
