...
//! with `enable_frontend_peek_sequencing` enabled for testing frontend peek performance.
...
impl SqlListener {
    ...
    pub async fn serve_sql(
        ...
        task::spawn(|| format!("{}_sql_server", label), {
            ...
            // Read optional CockroachDB URL for the real timestamp oracle.
            let metadata_url = std::env::var("METADATA_BACKEND_URL").ok();

            // Create the mock adapter service with frontend peek sequencing enabled.
            let adapter_service = MockAdapterService::new(&metrics_registry, metadata_url).await;
            ...
            tracing::info!("running pgwire server with frontend peek sequencing");
