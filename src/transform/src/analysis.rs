...
pub mod common {
    ...
    pub struct Derived {
        ...
        /// Uses a Vec instead of BTreeMap since there are at most ~9 analysis types,
        /// making linear search faster than tree traversal due to cache locality.
        analyses: Vec<(TypeId, Box<dyn AnalysisBundle>)>,
    ...
    impl Derived {
        ...
        pub fn results<A: Analysis>(&self) -> &[A::Value] {
            ...
            for (id, bundle) in &self.analyses {
                if *id == type_id {
    ...
    impl<'a> DerivedBuilder<'a> {
        ...
        pub fn visit(mut self, expr: &MirRelationExpr) -> Derived {
            ...
            for id in self.result.order.iter() {
                if let Some(pos) = self.result.analyses.iter().position(|(tid, _)| tid == id) {
                    let (tid, mut bundle) = self.result.analyses.swap_remove(pos);
                    bundle.analyse(&rev_post_order[..], &self.result);
                    self.result.analyses.push((tid, bundle));
                }
            }
